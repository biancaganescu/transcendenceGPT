...
out_dir = None
BIG_STEPS = [1000, 2000, 3000, 4000, 5000]
...
STEPS = BIG_STEPS if "2" in out_dir else SMALL_STEPS

for step in STEPS:
    print("\n")
    print("=====================TESTING CHECKPOINT AT STEP " + str(step) +"=====================")
    ckpt_path = os.path.join(out_dir, 'ckpt_step' + str(step) + '.pt')
    ...
    for shots in range(1, 31):
        print("\n")
        print("TESTING FOR " + str(shots) + " SHOTS")
        test_data = torch.load("./data/in_context_learning_test/len_" + str(shots) + "_test.pt")[0]
        ...
        # Print results:
        print("\n")
        print("RESULTS FOR CHECKPOINT " + str(step) + " WITH " + str(shots) + " ICL SHOTS")
